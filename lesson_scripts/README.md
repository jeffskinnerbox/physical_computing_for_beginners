...
| Class 3 lesson script | `class-03-lesson-script.md` | Walkthrough for a DRV8833 dual H-bridge motor driver and DC gearbox motors; the reader attempts open-loop square/circle driving and experiences dead-reckoning drift firsthand. Also introduces wheel odometry (an IR optocoupler per wheel) and the first version of a Pico-hosted rover status website, plus an optional stretch: drive straight with wheel feedback (Phase 5) and tune its `KI`/`MAX_TRIM` by experiment (Phase 6). |
